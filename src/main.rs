...
fn main() {
    ...
    for n in [20, 25, 30].iter() {
        ...
        let res0 = fast_ackermann(m, *n);
        let e0 = PreciseTime::now();
        println!("ack_with_cache -> {} took {}", res0, s0.to(e0));

        let s1 = PreciseTime::now();
        let res1 = fast_ackermann(m, *n);
        let e1 = PreciseTime::now();
        println!("fast_ackermann -> {} took {}", res1, s1.to(e1));

        let s2 = PreciseTime::now();
        let res2 = vec_ack(m, *n);
        let e2 = PreciseTime::now();
        println!("vec_ack -> {} took {}", res2, s2.to(e2));

        let s3 = PreciseTime::now();
        let res3 = arr_ack(m, *n);
        let e3 = PreciseTime::now();
        println!("arr_ack -> {} took {}", res3, s3.to(e3));
    }
}
...
fn a_use_p2(m: u64, n: u64, r_use: Vec<u64>) -> Vec<u64> {
    if m == 0 {
        vec![n + 1]
    } else if m == 1 {
        vec![n + 2]
    } else if n == 0 {
        (0..m).fold(vec![1], |v, k| a_use_p2(k, 1, v))
    } else if n == 1 {
        let v11 = r_use[0];
        let mut v2 = r_use.clone();
        for k in 2..(v11 + 1) {
            v2 = a_use_p2(m - 1, k, v2);
        }
        let v3 = v2[0];
        v2.insert(0, v3);
        v2.insert(1, v11);
